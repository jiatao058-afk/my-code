  ...
  <div class="index">
    <div class="index-box">
      <div style="text-align:center;padding:4px">
        <el-button size="small" type="danger" @click="cheatMaxAll" :disabled="cheated">一键满级</el-button>
        <el-button size="small" type="warning" @click="decomposeAll">一键分解全部</el-button>
        <span v-if="cheated" style="color:green;margin-left:8px">已满级</span>
      </div>
      <div class="story">
        <p v-html="storyText" />
      </div>
                ...
                <el-tab-pane :label="i.name" :name="i.type" v-for="(i, k) in backPackItem" :key="k">
                  <div class="inventory-content">
                    <div v-if="player.inventory.length">
                      <div v-if="selectedDecomposeIds.size > 0" class="batch-buttons">
                        <el-button size="small" type="danger" @click="decomposeSelected">
                          分解选中 ({{ selectedDecomposeIds.size }})
                        </el-button>
                        <el-button size="small" @click="selectedDecomposeIds.clear()">取消选择</el-button>
                      </div>
                      <template v-for="item in player.inventory" :key="item.id">
                        <el-popover placement="bottom" :title="item.name" :width="300" trigger="hover">
                          <template #reference>
                            <span class="inventory-item-wrapper">
                              <el-checkbox
                                v-if="item.type == i.type"
                                :model-value="selectedDecomposeIds.has(item.id)"
                                @change="(checked) => toggleDecompose(item.id, checked)"
                                :disabled="item.lock"
                                size="small"
                              />
                              <tag
...
<script setup>
  ...
  const cheatMaxAll = () => {
    ...
    p.level = 144
    p.attack = 99999999; p.health = 99999999; p.maxHealth = 99999999
    p.defense = 99999999; p.critical = 0.99; p.dodge = 0.99
    p.points = 999; p.score = 99999999
    p.props.money = 99999999; p.props.flying = 999; p.props.rootBone = 999
    p.props.currency = 999; p.props.cultivateDan = 999; p.props.strengtheningStone = 99999
    p.cultivation = p.maxCultivation || 99999999
    p.reincarnation = 99; p.backpackCapacity = 999
    cheated.value = true
    gameNotifys({ title: '搞定', message: '修为装备全满级，刷新页面后依然有效' })
  }
  // 一键分解全部未锁定装备
  const decomposeAll = () => {
    const items = player.value.inventory.filter(item => !item.lock)
    if (items.length === 0) {
      gameNotifys({ title: '提示', message: '没有可分解的装备（全部已锁定或无装备）' })
      return
    }
    ElMessageBox.confirm(
      `确定要分解背包中全部 ${items.length} 件未锁定装备吗?`,
      '一键分解全部',
      { center: true, cancelButtonText: '取消', confirmButtonText: '确定' }
    ).then(() => {
      let totalStones = 0
      player.value.inventory = player.value.inventory.filter(item => item.lock)
      ...
      gameNotifys({ title: '分解完成', message: `分解 ${items.length} 件装备，获得 ${totalStones} 个炼器石` })
    }).catch(() => {})
  }
  // 装备信息弹窗
  const inventoryShow = ref(false)
  // 多选分解
  const selectedDecomposeIds = ref(new Set())
  const toggleDecompose = (id, checked) => {
    if (checked) {
      selectedDecomposeIds.value = new Set([...selectedDecomposeIds.value, id])
    } else {
      const next = new Set(selectedDecomposeIds.value)
      next.delete(id)
      selectedDecomposeIds.value = next
    }
  }
  const decomposeSelected = () => {
    const ids = selectedDecomposeIds.value
    if (ids.size === 0) return
    const items = player.value.inventory.filter(item => ids.has(item.id) && !item.lock)
    if (items.length === 0) {
      gameNotifys({ title: '提示', message: '没有可分解的装备' })
      return
    }
    ElMessageBox.confirm(
      `确定要分解选中的 ${items.length} 件装备吗?`,
      '批量分解',
      {
        center: true,
        cancelButtonText: '取消',
      ...
      player.value.props.strengtheningStone += totalStones
      selectedDecomposeIds.value = new Set()
      gameNotifys({
        title: '分解完成',
        message: `成功分解 ${items.length} 件装备，获得 ${totalStones} 个炼器石`
      })
    }).catch(() => {})
  }
  // 炼器弹窗
